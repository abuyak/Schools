...
# FIX: Removed POSTS_PER_PASS cap â€” we now process ALL visible items each pass.
# FIX: Increased stagnant pass limit â€” Facebook feed can be slow to load new posts.
...
def scrape_posts() -> list[dict]:
    ...
    # FIX: Track the highest aria-posinset value we've processed, so we always
    # advance forward through the feed and never re-scan old DOM items.
    # This replaces the broken start_idx / count / POSTS_PER_PASS index logic.
    ...
    with sync_playwright() as p:
        ...
        while True:
            ...
            print(
                ...
                f"highest posinset processed: {highest_processed_posinset}"
            )
            ...
            # FIX: Iterate over ALL visible items, skipping only those whose
            # aria-posinset we have already processed.  No arbitrary cap.
            before = len(posts)
            for idx in range(total_visible):
                item = items.nth(idx)

                try:
                    posinset_str = item.get_attribute("aria-posinset") or "0"
                    posinset = int(posinset_str)
                except Exception:
                    posinset = 0

                # Already handled â€” skip without touching the DOM.
                ...
                # Advance the watermark even for posts we skip.
                if posinset > 0:
            ...
            # FIX: Scroll to the bottom of whatever is loaded, then wheel further.
            # Larger delta + longer wait gives Facebook time to fetch the next batch.
            try:
            ...
            if len(posts) == before:
                ...
                if stagnant_passes >= STAGNANT_PASS_LIMIT:
                    # FIX: Do NOT call open_group_page() here â€” that navigates back to the
                    # top and throws away all scroll progress.  Instead use JS scrollTo to
                    # push the page to the very bottom and wait for more content.
