...
function buildComparisonBlock(schools) {
  ...
  if (hasKS2) {
    ...
    rows.push(['Reading progress',  s => { const v = _nsField(s, 'READPROG_23'); const b = _nsField(s, 'READPROG_DESCR_23'); return v != null ? v + ' (' + (b || '—') + ')' : '—'; }]);
    rows.push(['Maths progress',    s => { const v = _nsField(s, 'MATPROG_23');   const b = _nsField(s, 'MATPROG_DESCR_23');   return v != null ? v + ' (' + (b || '—') + ')' : '—'; }]);
  }
...
export function renderPartAComparison(schools) {
  ...
  if (hasKS2) {
    ...
    sections.push({
      ...
      body: buildTable4('National', [
        ['Reading progress', s => { const v = _nsField(s, 'READPROG_23'); const b = _nsField(s, 'READPROG_DESCR_23'); return v != null ? v + ' (' + (b || '—') + ')' : '—'; }, '0'],
        ['Writing progress', s => { const v = _nsField(s, 'WRITPROG_23'); const b = _nsField(s, 'WRITPROG_DESCR_23'); return v != null ? v + ' (' + (b || '—') + ')' : '—'; }, '0'],
        ['Maths progress', s => { const v = _nsField(s, 'MATPROG_23');   const b = _nsField(s, 'MATPROG_DESCR_23'); return v != null ? v + ' (' + (b || '—') + ')' : '—'; }, '0'],
      ]),
      flag: 'none',
    });
