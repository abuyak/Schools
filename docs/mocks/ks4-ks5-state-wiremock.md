...
- Sixth form provision grade if applicable
...
Heading: `## A3. Observations` · Flag: `green` if Attainment 8 >national+10, P8 >0.5, or A-level grade above national; `red` if below by same margins

Bullet list, 3–4 bullets:
- Overall attainment vs national and LA benchmarks
- Progress 8 / VA score — direction, significance, disadvantaged gap
- Multi-year trend from results-over-time table
- KS5: A-level grade and progress, comment on facilitating subjects %
...
Heading: `## A4. Observations` · Flag: `red` if FSM >30% secondary or EHC >6%, else `none`
